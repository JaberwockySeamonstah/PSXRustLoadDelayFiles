...
pub(crate) fn update_controller() {
    let mut serial_connection = SerialConnection::activate();
        process_port(&mut serial_connection, unsafe{&raw mut CONTROLLERS_A[0]});
...
fn process_port(serial_connection: &mut SerialConnection, port_slots: *mut ControllerSlot) {
    for idx in 0..CONTROLLER_SLOT_COUNT {
        let slot = unsafe{&mut *port_slots.wrapping_add(idx)};
...
fn process_controller(serial_connection: &mut SerialConnection, controller: *mut Option<RawController>, configuration: *const Option<Configuration>, slot: u8) {
    if let Some(existing_controller) = unsafe{&mut *controller} {
        // An existing controller needs to be state checked
        if let Err(_) = process_existing_controller(serial_connection, &raw mut *existing_controller, configuration, slot) {
        ...
        match serial_connection.get_controller_state_form_slot(slot, 0, 0) {
            Ok((id, button_state, special)) => {
                unsafe{*controller = Some(RawController::new(id, button_state, special))};           
            }
...
fn process_existing_controller(serial_connection: &mut SerialConnection, controller: *mut RawController, configuration: *const Option<Configuration>, slot: u8) -> Result<(), SerialConnectionError> {
    match unsafe{&*controller}.get_state() {
        ControllerState::New    => {
            // TODO: Investigate this more! Is the problem the `match` followed by the `if`?
            //unsafe { asm!("nop"); }
            if unsafe{*configuration}.is_some() {
                if serial_connection.enter_config_mode(slot, true).is_ok() {
                    unsafe{&mut *controller}.set_config_mode();
                    return Ok(());
                }
            ...
            unsafe{&mut *controller}.set_stable();
            Ok(())
        },
        ControllerState::InConfigMode(current_config) => {
            if let Some(requested_config) = unsafe{*configuration} {
                ...
                if current_config.analog_mode() != requested_config.analog_mode() || current_config.lock_mode() != requested_config.lock_mode() {
                    ...
                    unsafe{&mut *controller}.set_configuration(current_config.with_analog_mode(requested_config.analog_mode()).with_lock_mode(requested_config.lock_mode()));
        ...
        ControllerState::Stable => {
            ...
            unsafe{&mut *controller}.update(id, button_state, special);
            Ok(())
        },
    }
}